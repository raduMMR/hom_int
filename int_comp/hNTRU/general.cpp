...
int ComputeFactorDegree(int m){
	...
	// cout<< "ComputeFactorDegree = " << p << endl;
...
void myMemMeas::Start(){
	sysinfo(&sinf);
	start_mem = sinf.freeram;
}

void myMemMeas::Stop(){
	sysinfo(&sinf);
	end_mem = sinf.freeram;
}

void myMemMeas::ShowMemory(string s){
	cout << dec << "Start:\t" << start_mem << endl;
	cout << dec << "Stop :\t" << end_mem << endl;

	cout << s << (start_mem-end_mem)/1024.0/1024/1024 << " GB\t\t\t" << (start_mem-end_mem)/1024.0/1024 << " MB" << endl;
}
