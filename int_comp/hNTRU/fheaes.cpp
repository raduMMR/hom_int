...
void FheAES::LTVKeySetUp(){
	...
	n = new ntru(gp.q, gp.B, gp.N_PolyDegree, gp.M_CycDegree);
	n->IOReadModulus();
	n->IOReadModulusExt();
...
void DecryptAndPrint(ntru *n, ZZX *Bits, int j){
	
	ZZX tt[128];
	for(int t=0; t<128; t++)
		tt[t] = n->Decrypt(Bits[t], j);
	PrintChar(tt, 16);
}
...
void FheAES::RelinBits(ZZX *bits, int num){
	for(int i=0; i<num; i++)
		n->PolyCoeffReduce(bits[i], bits[i], qIndex);

	for(int i=0; i<num; i++)
		bits[i]	= n->RelinRingFFT(bits[i], qIndex, 0);
...
void FheAES::ModSwitchBits(ZZX *bits, int num){
	...
	for(int i=0; i<num; i++){
		bits[i] = n->ModSwitch(bits[i], qIndex);
		n->CoeffReduce(bits[i], bits[i], qIndex+1);
	}
}
...
void FheAES::SetKeys(){
	...
	for(int s=0; s<11; s++)
		for(int i=0; i<128; i++)
			RoundKeys[s][i] = n->Encrypt(RoundKeys[s][i], 0);

	for(int s=0; s<11; s++)
			for(int i=0; i<128; i++)
				n->CoeffReduce(RoundKeys[s][i], RoundKeys[s][i], 4*s);
}
